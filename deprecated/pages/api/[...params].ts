...
export async function SEARCH_ANIME_BY_IMAGE(payload: string) {
  const data = await axios
    ...
    .then(async (response) => {
      ...
      const similarity = `${(response.data.result[0].similarity * 100).toFixed(0)}%`
...
export async function SSG_HOME_PAGE() {
  async function getAnimeRandom() {
    ...
    const newArray = myRandomFunctionAnime.slice(1, 6).map((x) => parseInt(x, 10))
...
export default async function WhatAnimeAPI(request: NextApiRequest, response: NextApiResponse) {
  ...
  if (request.method === 'GET') {
    if (params[0] === 'anime') {
      switch (params[1]) {
        case 'name':
          const ANIME_BY_NAME = await SEARCH_ANIME_BY_NAME(params[2], parseInt(params[3], 10))
